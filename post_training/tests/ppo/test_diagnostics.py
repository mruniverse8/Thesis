...
def test_rollout_stage_metrics_capture_realized_lengths_and_stage_breakdown() -> None:
    ...
    assert metrics["mean_trajectory_length"] == pytest.approx(2.0)
    assert metrics["max_trajectory_length"] == pytest.approx(3.0)
    assert metrics["fraction_rollouts_planned_trajectory_length_2_plus"] == pytest.approx(1.0)
    assert metrics["fraction_rollouts_trajectory_length_2_plus"] == pytest.approx(2.0 / 3.0)
    assert metrics["fraction_rollouts_trajectory_length_3_plus"] == pytest.approx(1.0 / 3.0)
    assert metrics["fraction_rollouts_reaching_planned_trajectory_length"] == pytest.approx(0.0)
    assert metrics["trajectory_length_1_fraction"] == pytest.approx(1.0 / 3.0)
    assert metrics["trajectory_length_2_fraction"] == pytest.approx(1.0 / 3.0)
    assert metrics["trajectory_length_3_plus_fraction"] == pytest.approx(1.0 / 3.0)
...
def test_rollout_stage_metrics_return_zeroed_defaults_for_empty_batches() -> None:
    ...
    assert metrics["mean_trajectory_length"] == pytest.approx(0.0)
    assert metrics["fraction_rollouts_trajectory_length_2_plus"] == pytest.approx(0.0)
    assert metrics["fraction_rollouts_trajectory_length_3_plus"] == pytest.approx(0.0)
