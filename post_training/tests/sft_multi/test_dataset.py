...
from post_training.sft_multi.collator import MultiMoleculeCollator
from post_training.sft_multi.dataset import MultiMoleculeDataset, build_multi_molecule_processed_record
...
def test_multi_molecule_dataset_and_collator_emit_staged_targets() -> None:
    ...
    item = dataset[0]
    assert item["target_text"] == "<bom>[C][O]<eom> <bom>[C][C]<eom>"
    ...
    assert batch["target_texts"] == ["<bom>[C][O]<eom> <bom>[C][C]<eom>"]
