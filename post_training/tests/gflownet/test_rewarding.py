...
def test_score_stage_terminal_reward_can_disable_invalid_ngram_fallback() -> None:
    summary = score_stage_terminal_reward(
        None,
        invalid_candidate_text="[C][C][Bad]",
        targets=["[C][C][O]"],
    ...
    assert summary.is_valid_terminal is False
    assert summary.reward_breakdown["is_valid"] is False
    assert summary.reward_breakdown["match_reward"] == pytest.approx(0.0)
    assert summary.reward_breakdown["total_reward"] == pytest.approx(0.0)
    assert summary.reward_breakdown["amplified_reward"] == pytest.approx(0.0)
    assert summary.terminal_reward == pytest.approx(2.5e-5)
