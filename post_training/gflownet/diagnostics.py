...
def termination_reason_metrics(
    ...
    if not trajectories:
        return {
            "termination_fraction_stop_token": 0.0,
            "termination_fraction_eos_token": 0.0,
            "termination_fraction_max_stage_new_tokens": 0.0,
            "termination_fraction_max_sequence_length": 0.0,
        }
    ...
    metrics = {
        "termination_fraction_stop_token": counts.get("stop_token", 0) / total,
        "termination_fraction_eos_token": counts.get("eos_token", 0) / total,
        "termination_fraction_max_stage_new_tokens": counts.get("max_stage_new_tokens", 0)
        / total,
        "termination_fraction_max_sequence_length": counts.get("max_sequence_length", 0)
        / total,
    }
    for reason, count in sorted(counts.items()):
        metric_key = (
            f"termination_fraction_{reason.strip().lower().replace('-', '_').replace(' ', '_')}"
        )
