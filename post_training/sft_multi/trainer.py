...
from src.io_utils import PROJECT_ROOT, load_yaml, resolve_path, set_seed, write_json
...
from .checkpointing import prepare_sft_output_dir, save_sft_checkpoint, write_sft_history
from .collator import MultiMoleculeCollator
from .dataset import MultiMoleculeDataset
from .encoding import prepare_sft_tokenizers


def build_multi_molecule_dataloader(
...
def run_multi_molecule_sft(config: dict[str, Any]) -> dict[str, Any]:
    ...
    validation_loader = build_multi_molecule_dataloader(
    ...
    output_dir = prepare_sft_output_dir(
...
def train_multi_molecule_sft(config: dict[str, Any]) -> dict[str, Any]:
    return run_multi_molecule_sft(config)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train BioT5+ on staged description-to-multi-SELFIES SFT.",
    )
    parser.add_argument(
        "--config",
