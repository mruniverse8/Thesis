...
from .ppo_sequence import build_stage_prefix
from .ppo_types import RolloutGenerationConfig, StageTrajectory
from .reward_adapter import score_stage_candidate
...
def encode_decoder_prefix(
    ...
    *,
    ...
    return torch.cat([start_tensor, prefix_ids.to(device)], dim=1)
...
def sample_stage(
    ...
    *,
    ...
    separator_token_id = tokenizer.convert_tokens_to_ids(generation_config.molecule_separator_token)
    ...
    with torch.no_grad():
        for _ in range(generation_config.max_stage_new_tokens):
            ...
            if next_token_id == separator_token_id:
                stop_token = generation_config.molecule_separator_token
    ...
    body_token_ids = list(action_token_ids)
    if stop_token is not None and body_token_ids:
        body_token_ids = body_token_ids[:-1]

    sampled_selfies = ""
    if body_token_ids:
        sampled_selfies = normalize_selfies_text(
            tokenizer.decode(
                body_token_ids,
    ...
    return {
        "sampled_selfies": sampled_selfies or None,
...
def sample_rollout_for_example(
    ...
    *,
    ...
    for stage_index in range(1, generation_config.max_molecules_per_sequence + 1):
        prefix_text = build_stage_prefix(
            previous_selfies,
            separator_token=generation_config.molecule_separator_token,
        )
        ...
        reward_breakdown = score_stage_candidate(
        ...
        if trajectory.stop_token == EOM_TOKEN:
            break
